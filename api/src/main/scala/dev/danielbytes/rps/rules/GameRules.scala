...
trait GameRules {
   ...
   */
  def gameTurn(game: Game, userId: UserId, from: Point, to: Point): Either[RuleViolationError, GameWithMoveSummary] = {
    for {
      moveResult <- moveRules.moveToken(game, userId, from, to)
      gameResult <- handleMove(game, from, to, moveResult)
      result = gameResult.copy(game = gameResult.game.copy(currentPlayerId = game.otherPlayer.id))
    } yield result
  }
   ...
   */
  def gameStatus(game: Game): Either[GameStateError, GameStatus] = {
    val state = game.playerList.map(getPlayerState(game, _))
  ...
  private def handleMove(
    ...
    from: Point,
    to: Point,
    moveResult: PotentialMove): Either[RuleViolationError, GameWithMoveSummary] = {
    moveResult match {
  ...
  private def handleCombat(
    ...
    to: Point,
    attacker: Token,
    defender: Token): Either[RuleViolationError, GameWithMoveSummary] = {
    combatRules.attackPlayer(attacker, defender) flatMap {
      case r: AttackerWinsCombat =>
        Right(movePoint(game, from, to).copy(move = Some(MoveSummary(game.currentPlayerId, from, to, Some(r)))))
   ...
   * @return The updated game state
   */
  private def movePoint(game: Game, from: Point, to: Point): GameWithMoveSummary = {
    val tokens = game.board.tokens
    val token = tokens(from)

    GameWithMoveSummary(
      game.copy(board = game.board.copy(tokens = (tokens - from - to) + (to -> token))),
   ...
   * @return The updated game state
   */
  private def removePoints(game: Game, points: Iterable[Point]): Game = {
    val tokens = game.board.tokens

    game.copy(board = game.board.copy(tokens = tokens -- points))
  }
   ...
   */
  private def getPlayerState(game: Game, player: Player): PlayerState = {
   ...
   */
  private case class PlayerState(userId: UserId, hasFlag: Boolean, canMove: Boolean)
}
