...
private[repositories] object SerializationModels {
  ...
  object GameModel {
    ...
    def apply(model: Game): GameModel =
      GameModel(
    ...
    private def toString(s: StartPosition) =
      s match {
    ...
    private def toStartPosition(s: String): Try[StartPosition] =
      Try {
        s match {
    ...
    private def toTokenType(s: String): Try[TokenType] =
      Try {
        Token.types
          .find(_.name == s)
    ...
    private def toPoint(s: String): Try[Point] =
      Try {
        s.split(":") match {
          case Array(x, y) =>
    ...
    private def toTokens(t: Map[String, String]): Try[Map[Point, Token]] =
      Try {
