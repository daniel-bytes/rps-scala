 ...
 */
class RedisGameRepository(val client: RedisClient)(implicit val ec: ExecutionContext)
  extends GameRepository
  with RedisRepository {
  ...
  override def set(id: GameId, model: Game): Future[Unit] = {
    for {
